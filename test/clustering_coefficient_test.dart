...
void main() {
  group('Local Clustering Coefficient', () {
    test(
      'Clustering coefficient is 0.0 for nodes with less than 2 neighbors',
      () {
    ...
    test(
      'Clustering coefficient is 0.0 when neighbors are completely disconnected',
      () {
        final graph = DependencyGraph();
        graph.addEdge('A', 'B');
        graph.addEdge('A', 'C');
    ...
    test(
      'Clustering coefficient is 1.0 when neighbors are fully connected (clique)',
      () {
        final graph = DependencyGraph();
        graph.addEdge('A', 'B');
        graph.addEdge('A', 'C');
    ...
    test(
      'Clustering coefficient is correct for partially connected neighbors',
      () {
        final graph = DependencyGraph();
        graph.addEdge('A', 'B');
        graph.addEdge('A', 'C');
