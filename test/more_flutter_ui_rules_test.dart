...
void main() {
  group('UnreleasedControllersRule', () {
    ...
    const rule = UnreleasedControllersRule(config: config);

    test(
      'flags controllers declared in State class when dispose() is missing',
      () {
    ...
    test(
      'flags only the controllers that are NOT disposed inside dispose()',
      () {
        const projectContext = ProjectContext(
          projectPath: '.',
          isFlutterProject: true,
      ...
      ''';
        ...
        expect(
          findings.first.evidence['controller'],
          equals('_scrollController'),
        );
      },
    );
  ...
  group('HardcodedUiTokensRule', () {
    ...
    const rule = HardcodedUiTokensRule(config: config);

    test(
      'flags raw Color and TextStyle instantiations inline in build method',
      () {
        const projectContext = ProjectContext(
          projectPath: '.',
          isFlutterProject: true,
  ...
  group('SetStateInComplexBuildsRule', () {
    ...
    const rule = SetStateInComplexBuildsRule(config: config);
  ...
  group('MissingConstConstructorsRule', () {
    const config = RuleConfig(enabled: true, severity: Severity.warning);
    const rule = MissingConstConstructorsRule(config: config);
