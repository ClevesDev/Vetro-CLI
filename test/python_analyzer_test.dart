...
void main() {
  group('Python Analyzer Rules - Unit Tests (Mock ASTs)', () {
    test(
      'PySemanticDuplicationRule - detects similar function shapes',
      () async {
        ...
        final roots = {'/src/file1.py': file1, '/src/file2.py': file2};

        const rule = PySemanticDuplicationRule(
          config: RuleConfig(
            enabled: true,
    ...
    test(
      'PythonAdapter - only extracts top-level/direct class methods and ignores nested functions',
      () {
        ...
        final root = makeMockNode(type: 'Module', children: [outerFn]);
