...
void main() {
  group('TypeScript Analyzer Rules - Unit Tests (Mock ASTs)', () {
    test('TsCyclomaticComplexityRule - flags complex functions', () {
      ...
      const rule = TsCyclomaticComplexityRule(config: config);
    ...
    test('TsCognitiveComplexityRule - counts nesting level correctly', () {
      ...
      const rule = TsCognitiveComplexityRule(config: config);
    ...
    test('TsLowEntropyRule - flags repetitive / low-entropy structures', () {
      ...
      const rule = TsLowEntropyRule(config: config);
      ...
      final repeatedIdentifiers = List.generate(
        15,
        (_) => makeMockNode(type: 'Identifier', extra: {'name': 'x'}),
      );
    ...
    test('TsIntentGapRule - flags complex functions lacking explanations', () {
      ...
      const rule = TsIntentGapRule(config: config);
    ...
    test(
      'TsLowCohesionRule - flags class with disjoint method vocabularies',
      () {
        const config = RuleConfig(
          enabled: true,
          severity: Severity.warning,
        ...
        const rule = TsLowCohesionRule(config: config);
        ...
        final root = makeMockNode(type: 'File', children: [classNode]);
    ...
    test(
      'TsTightCouplingRule & TsCircularDependencyRule - detects cross-file rules',
      () async {
        ...
        const couplingRule = TsTightCouplingRule(
          config: RuleConfig(
            enabled: true,
            severity: Severity.warning,
        ...
        const circularRule = TsCircularDependencyRule(
          config: RuleConfig(enabled: true, severity: Severity.warning),
        );

        final couplingFindings = await couplingRule.analyzeProject(
          projectRoots,
          {},
        );
        ...
        final circularFindings = await circularRule.analyzeProject(
          projectRoots,
          {},
        );
        ...
        expect(
          circularFindings.first.message,
          contains('fileA.ts -> fileB.ts -> fileC.ts -> fileA.ts'),
        );
      },
    );

    test(
      'TsSemanticDuplicationRule - detects similar function shapes',
      () async {
        ...
        final roots = {'/src/file1.ts': file1, '/src/file2.ts': file2};

        const rule = TsSemanticDuplicationRule(
          config: RuleConfig(
            enabled: true,
            severity: Severity.warning,
